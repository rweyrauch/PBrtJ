...
public final class Spectrum implements ArithmeticOps<Spectrum> {
    ...
    public static float AverageSpectrumSamples(float[] lambda, float[] vals, float lambdaStart, float lambdaEnd) {
        ...
        for (; i + 1 < n && lambdaEnd >= lambda[i]; ++i) {
            float segLambdaStart = Math.max(lambdaStart, lambda[i]);
            float segLambdaEnd = Math.min(lambdaEnd, lambda[i + 1]);
            sum += 0.5 * (interp(lambda, vals, segLambdaStart, i) + interp(lambda, vals, segLambdaEnd, i)) *
    ...
    public static float InterpolateSpectrumSamples(float[] lambda, float[] vals, float l) {
        ...
        // TODO: verify the lambda expression is correct!!!
        int offset = Pbrt.FindInterval(n, index -> lambda[index] <= l);
    ...
    public static final float CIE_Y_integral = 106.856895f;

    public static float[] CIE_X = {
    ...
    public static float[] CIE_lambda = {
